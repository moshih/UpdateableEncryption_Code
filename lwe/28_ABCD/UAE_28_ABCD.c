...
void main_uae_timings() {
    ...
    uint8_t *message = malloc(10*size);
    RAND_bytes(message,size);
    uint8_t *ctx1 = malloc(10*size);
    uint8_t *ctx2 = malloc(10*size);
    uint8_t *decrypted_message1 = malloc(10*size+(PAD_SIZE));
    uint8_t *decrypted_message2 = malloc(10*size+(PAD_SIZE));
    ...
    printf("UAE 28_ABCD Size:%d Runs:%u\n gen_key:        %f %d\n encrypt:        %f %d\n regen_key:    %f %d\n re_encrypt: %f %d\n\n ",
        size,runs,
        gen_cycles,runs,
        encrypt_cycles,runs,
...
void main_uae_timings_avx() {
    ...
    for (int j=0; j<runs; j++){
        ...
        // Encrypt
        begin = clock();
        int padded_size = UAE_Encrypt_avx(AE_key1,message, &ctx_hat1, ctx1, size);
        end = clock();
        encrypt_cycles += (double)(end - begin);
        ...
        for (int i=0; i<total_re_encrypts; i++){
            if (i%2 == 0){
                ...
                // ReEncrypt
                begin = clock();
                re_encrypt_length = UAE_ReEncrypt_avx(&delta1, &ctx_hat1, ctx1, &ctx_hat2, ctx2, padded_size);
                end = clock();
                re_encrypt_cycles += (double)(end - begin);

                // Decrypt
                begin = clock();
                int decrypted_size1 = UAE_Decrypt_avx(AE_key1,decrypted_message1, &ctx_hat1, ctx1, re_encrypt_length);
                end = clock();
                decrypt_cycles[i] += (double)(end - begin);
                ...
                begin = clock();
                UAE_Keygen(AE_key1);
                rekeygen_output = UAE_ReKeygen_avx(AE_key2,AE_key1, &ctx_hat2, &delta1);
                end = clock();
                regen_cycles += (double)(end - begin);

                // ReEncrypt
                begin = clock();
                re_encrypt_length = UAE_ReEncrypt_avx(&delta1, &ctx_hat2, ctx2, &ctx_hat1, ctx1, padded_size);
                end = clock();
                re_encrypt_cycles += (double)(end - begin);
                ...
                // Decrypt
                begin = clock();
                int decrypted_size2 = UAE_Decrypt_avx(AE_key2,decrypted_message2, &ctx_hat2, ctx2, re_encrypt_length);
                end = clock();
                decrypt_cycles[i] += (double)(end - begin);
            }
        }
