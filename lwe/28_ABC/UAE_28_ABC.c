...
void main_uae_timings() {
    ...
    uint8_t *message = malloc(100*size);
    RAND_bytes(message,size);
    uint8_t *ctx1 = malloc(100*size);
    uint8_t *ctx2 = malloc(100*size);
    uint8_t *decrypted_message1 = malloc(100*size+(PAD_SIZE));
    uint8_t *decrypted_message2 = malloc(100*size+(PAD_SIZE));
    ...
    printf("UAE 28_ABC Size:%d Runs:%u\n gen_key:        %f %d\n encrypt:        %f %d\n regen_key:    %f %d\n re_encrypt: %f %d\n\n ",
        size,runs,
        gen_cycles,runs,
        encrypt_cycles,runs,
