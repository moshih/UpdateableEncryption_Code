...
extern int64_t zetas_B[NEWHOPE_N/2];
extern int64_t zetas_inv_B[NEWHOPE_N/2];

/*************************************************
*Name:        montgomery_reduce_B
*
...
int64_t montgomery_reduce_B(__int128 a)
{
    ...
    u = a *(unsigned __int128) QINV_S64_B;

    t = (__int128) u *(__int128) QB;
    t = a - t;
    t >>= 64;
...
{
    return montgomery_reduce_B((__int128) a *b);
}

/*************************************************
 *Name:        barrett_reduce_B
 *
...
{
    __int128 t;
    const __int128 v = ((__int128) 1U << BARRETT_REDUCE_FACTOR) / (__int128) QB + 1;
...
void poly_basemul_B(poly_60 *r, const poly_60 *a, const poly_60 *b)
{
    ...
    {
        basemul(r->coeffs + 4 *i, a->coeffs + 4 *i, b->coeffs + 4 *i, zetas_B[NEWHOPE_N / 4 + i]);
        basemul(r->coeffs + 4 *i + 2, a->coeffs + 4 *i + 2, b->coeffs + 4 *i + 2, -zetas_B[NEWHOPE_N / 4 + i]);
    }
}
...
void poly_frommont_one_B(poly_60 *r)
{
    ...
    for (i = 0; i < NEWHOPE_N; i++)
        r->coeffs[i] = montgomery_reduce_B((__int128) r->coeffs[i]);
}
...
void poly_reduce(poly_60 *r)
{
    ...
    for (i = 0; i < NEWHOPE_N; i++)
        r->coeffs[i] = barrett_reduce_B(r->coeffs[i]);
}
...
void invntt(int64_t r[NEWHOPE_N])
{
    ...
    for (len = 2; len <= NEWHOPE_N / 2; len <<= 1)
    {
        for (start = 0; start < NEWHOPE_N; start = j + len)
        {
            ...
            for (j = start; j < start + len; ++j)
            {
                ...
                r[j] = barrett_reduce_B(t + r[j + len]);
                r[j + len] = t - r[j + len];
    ...
    for (j = 0; j < NEWHOPE_N; ++j)
        r[j] = fqmul(r[j], zetas_inv_B[NEWHOPE_N / 2 - 1]);
}
