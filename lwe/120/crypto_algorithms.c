...
/*************************************************
 *Name:        poly_uniform_ref_poly_120
 *
 ...
 *
 *Arguments:   - poly_120 *a: pointer to the poly_120 to be generated
 *             - const unsigned char *seed: pointer to the seed input
 *
 *Returns:     None
 **************************************************/
void poly_uniform_ref_poly_120(poly_120 *a, const unsigned char *seed)
{
    unsigned int ctr = 0;
    uint64_t val;
    uint64_t state[25];
    ...
    {
        if (iteration == 65536) printf("ERROR: poly_uniform_ref_poly_120: iteration hit its limit\n");
        ...
        for (j = 0; j < SHAKE128_RATE - SAMPLE_BLOCK_SIZE && coeffs_written < NEWHOPE_N; j += SAMPLE_BLOCK_SIZE)
        {
            ...
            for (int k = 0; k < SAMPLE_BLOCK_SIZE; k++) sample_1 |= ((uint128_t) buf[j + k] << (8 *j));
 ...
 *
 *Description: Takes in a 32 bytes seed, generates a poly_120 with shake128 
 *             uniformly random elements in mod Q. Differs than poly_uniform_ref_poly_120
 ...
 *Returns:     None
 **************************************************/
void poly_uniform_ref_noise_16_1_seeds_pseudo(poly_120 *a, const unsigned char *seed)
{
    uint64_t state[25];
    uint8_t buf[SHAKE128_RATE];
 ...
 *
 *Description: Takes in a poly_120 key_point_poly and seed of a poly_120, point_seed,
 *             generated the second poly_120 from point_seed, multiplies then as if they
 *             they are in the NTT domain, then computes the inverse NTT of the product.
 *
 *Arguments:   - poly_120 *key_point_poly: pointer to the poly_120 input
 *             - poly_120 *product_point_poly: pointer to the poly_120 product
 *             - const unsigned char *seed: pointer to the seed input
 *
 *Returns:     None
 **************************************************/
void create_product_point(poly_120 *key_point_poly, poly_120 *product_point_poly, unsigned char *point_seed, const int128_t *zetas, int128_t *zetas_inv)
{
    int i;
    ...
    poly_uniform_ref_poly_120(&point_poly, point_seed);

    poly_basemul(product_point_poly, key_point_poly, &point_poly, zetas);
    poly_invntt(product_point_poly, zetas_inv);
}
 ...
 *
 *Arguments:   - poly_120 *key_point_poly: pointer to the poly_120 key
 *             - uint8_t *buf: pointer to the message
 *             - unsigned int size: size of message in bytes
 *
 *Returns:     None
 **************************************************/
void kh_prf_encrypt(poly_120 *key_point_poly, uint8_t *buf, unsigned int size)
{
    int128_t zetas[2048];
    ...
    while (bytes_processed < size)
    {
        ...
        for (i = 0; i < NEWHOPE_N && bytes_processed < size; i++)
        {
            ...
            sample_1 = reduce_modq(sample_1 + (uint128_t) product_poly.coeffs[i] + (uint128_t) noise_poly.coeffs[i] - NOISE_MAX / 2);
 ...
 *
 *Arguments:   - poly_120 *key_point_poly_1: pointer to the original poly_120 key
 *             - poly_120 *key_point_poly_2: pointer to the new poly_120 key
 *             - uint8_t *test_array: pointer to the ciphertext
 *             - unsigned int size: size of ciphertext in bytes
 *
 *Returns:     None
 **************************************************/
void kh_prf_re_encrypt(poly_120 *key_point_poly_1, poly_120 *key_point_poly_2, uint8_t *test_array, unsigned int size)
{
    poly_120 diff_poly;

    for (int i = 0; i < NEWHOPE_N; i++) diff_poly.coeffs[i] = reduce_modq(key_point_poly_2->coeffs[i] - key_point_poly_1->coeffs[i]);
 ...
 *Name:        lwe_gen_key
 *
 *Description: Generated a random poly_120 with no input.
 *
 *Arguments:   - poly_120 *key_point_poly: pointer to the poly_120 generated
 *
 *Returns:     None
 **************************************************/
void lwe_gen_key(poly_120 *key_point_poly)
{
    uint8_t key_seed[32];
    RAND_bytes(key_seed, 32);
    poly_uniform_ref_poly_120(key_point_poly, key_seed);
}
 ...
 *             Assumes that the message has been padded to be a multiple of 25.
 *
 *Arguments:   - poly_120 *key_point_poly: pointer to the poly_120 key
 *             - uint8_t *buf: pointer to the message
 *             - unsigned int size: size of message in bytes
 ...
 *Returns:     None
 **************************************************/
int kh_prf_encrypt_2_5(poly_120 *key_point_poly, uint8_t *buf, uint8_t *out, unsigned int size)
{
    int128_t zetas[2048];
    int128_t zetas_inv[2048];
    ...
    while (bytes_processed < size)
    {
        ...
        for (i = 0; i < NEWHOPE_N && bytes_processed < size; i += 2)
        {
            ...
            for (int j = 3; j < SAMPLE_BLOCK_SIZE; j++) sample_2 |= ((uint128_t) buf[bytes_processed + j - 3 + 13] << (8 *j));
 ...
 **************************************************/
int kh_prf_decrypt_2_5(poly_120 *key_point_poly, uint8_t *buf, uint8_t *out, unsigned int size)
{
    int128_t zetas[2048];
    int128_t zetas_inv[2048];
    ...
    while (bytes_processed < size)
    {
        ...
        for (i = 0; i < NEWHOPE_N && bytes_processed < size; i += 2)
        {
            ...
            sample_1 = reduce_modq(sample_1 - (uint128_t) product_poly.coeffs[i]);
            sample_2 = reduce_modq(sample_2 - (uint128_t) product_poly.coeffs[i + 1]);

            for (int j = 3; j < SAMPLE_BLOCK_SIZE; j++) out[bytes_written + j - 2] = (sample_1 &((uint128_t) 255ul << (8 *j))) >> (8 *j);
            for (int j = 3; j < SAMPLE_BLOCK_SIZE; j++) out[bytes_written + j - 3 + 13] = (sample_2 &((uint128_t) 255ul << (8 *j))) >> (8 *j);
 ...
 *             to be empty.
 *
 *Arguments:   - poly_120 *key_point_poly: pointer to the delta of the new and old key
 ...
 **************************************************/
int kh_prf_re_encrypt_2_5(poly_120 *key_point_poly, uint8_t *buf, uint8_t *out, unsigned int size)
{
    int128_t zetas[2048];
    int128_t zetas_inv[2048];
...
int UAE_ReKeygen(int8_t *AE_key1, int8_t *AE_key2, UAE_lwe_ctx_header *ciphertext_hat, UAE_lwe_delta *delta)
{
    ...
    poly_120 poly_key1;
    memcpy(&poly_key1, &data_header.poly_key, sizeof(poly_120));
    lwe_gen_key(&data_header.poly_key);

    for (int i = 0; i < NEWHOPE_N; i++) delta->poly_key.coeffs[i] = reduce_modq(data_header.poly_key.coeffs[i] - poly_key1.coeffs[i]);
